...
queries = {
    'NR-AhR': [
        'PFOS aryl hydrocarbon receptor',
        'PFOS AhR activation',
        'perfluorooctane sulfonic AhR',
    ],
    'SR-MMP': [
        'PFOS mitochondrial membrane potential',
        'PFOS mitochondrial toxicity',
        'perfluorooctane sulfonic mitochondrial',
    ],
}
...
for endpoint, pmids in all_papers.items():
    ...
    print(f"  PFOS {endpoint}: {len(unique_pmids)} 篇不重复文献")
...
for endpoint, pmids in all_papers.items():
    ...
    cal.setdefault('PFOS', {})[endpoint] = {
        'value': 1.0,
        'confidence': confidence,
        'n_papers': count,
    }
    print(f"  PFOS {endpoint}: {count}篇文献, confidence={confidence}")
...
print("  最终校正表（PFOS）")
print("="*70)
for ep, data in cal.get('PFOS', {}).items():
